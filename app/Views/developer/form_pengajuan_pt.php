...
$(document).ready(function() {
    ...
    $('#formpengajuanpt').on('submit', function(e) {
        ...
        var npwppt = $('#npwp_pt').val();
        if (npwppt.length != 15 && npwppt.length != 16) {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'NPWP PT harus 15 atau 16 digit'
            });
            $('#npwp_pt').addClass('is-invalid');
            $('#spannpwp_pt').html('NPWP PT harus 15 atau 16 digit');
            return false;
        }
        var npwppj = $('#npwp_penanggung_jawab').val();
        if (npwppj.length != 15 && npwppj.length != 16) {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'NPWP Penanggung Jawab harus 15 atau 16 digit'
            });
            $('#npwp_penanggung_jawab').addClass('is-invalid');
            $('#spannpwp_penanggung_jawab').html('NPWP Penanggung Jawab harus 15 atau 16 digit');
            return false;
        }
        var ktppj = $('#ktp_penanggung_jawab').val();
        if (ktppj.length != 16) {
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'NIK KTP Penanggung Jawab harus 16 digit'
            });
            $('#ktp_penanggung_jawab').addClass('is-invalid');
            $('#spanktp_penanggung_jawab').html('NIK KTP Penanggung Jawab harus 16 digit');
            return false;
        }
        Swal.fire({
            title: 'Mohon Tunggu',
        ...
        $.ajax({
            url: "<?= site_url('developer/pengajuan_pt_ajax') ?>",
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
