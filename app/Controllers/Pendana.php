...
class Pendana extends BaseController
{
    ...
    public function danai_pengajuan()
    {
        ...
        if($pengajuan['submited_status'] != 6){
            return $this->response->setJSON([
                'status' => 'error',
                'message' => [
    ...
    public function kembalikan_by_pendana() 
    {
        if(!$this->request->isAJAX()){
            return $this->response->setJSON(['message' => 'Invalid request'])->setStatusCode(400);
        }
        ...
        if (!$validation->withRequest($this->request)->run()) {
            return $this->response->setJSON([
                'status' => 'error',
                'message' => [
                    'simpan' => $validation->getError('keteranganpenolakan')
                ],
                'csrfHash' => csrf_hash(),
                'csrfToken' => csrf_token(),
                'uuid' => $uuid
        ...
        $data = [
            ...
            'submited_time' => $datetime,
            'submited_by' => session()->get('uuid'),
            'keteranganpenolakanpendana' => $this->request->getPost('keteranganpenolakan')
        ];
        ...
        try {
            ...
            setNotifikasi($uuiddeveloper, 'Pengajuan Dana', 'Pengajuan dana dikembalikan oleh Pendana karena '.$this->request->getPost('keteranganpenolakan'), '/developer/monitoring_pengajuan_dana');
            setNotifikasi(env('uuidapprover'), 'Pengajuan Dana', 'Pengajuan dana '.$developer['nama'].' telah dikembalikan oleh Pendana', '/approver/approval_dana/'.$uuiddeveloper);
